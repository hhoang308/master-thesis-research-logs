...
- một file pdf là một chuỗi các byte, mỗi byte là một kí tự và mỗi kí tự được chia thành 3 loại như sau: ký tự thường (regular characters), ký tự khoảng trắng (white space, bao gồm: null, tab, line feed, carriage return, space, ...) và ký tự phân cách (delimiter, dùng để phân tích các đối tượng như tên, mảng,...; bao gồm các kí tự: (, ), <, >, ...).
...
#### 7.4 Filters
- các stream object có thể được nén hoặc mã hóa, các bộ lọc phổ biến bao gồm:
  - ASCIIHexDecode, ASCII85Decode: mã hóa văn bản
  - LZWDecode, FlateDecode: nén dữ liệu
  - DCTDecode: nén ảnh JPEG
  - Crypt: mã hóa bảo mật

#### 7.5 File Structure
- một file pdf được coi là hợp lệ bao gồm 4 thành phần chính:
  - header
  - body
  - xref (bảng tham chiếu chéo)
  - trailer
- khi sửa tệp, pdf cho phép ghi thêm dữ liệu mới vào cuối tệp (body mới, xref section mới, trailer mới) thay vì ghi đè toàn bộ, giúp lưu nhanh hơn

#### 7.7 Document Structure
- document catalog: gốc của phân cấp đối tượng trong tài liệu (truy cập qua khóa /Root trong Trailer)
- page tree: cấu trúc cây để quản lý các trang, giúp truy cập nhanh trong tài liệu lớn

#### 7.8 Content Stream and Resources
- đây là nơi chứa các lệnh để "vẽ" nội dung, nó sử dụng ký pháp hậu tố với 
