...
- DynamoRIO là một dynamic binary instrumentation.
...
1. Mục tiêu là phát triển 1 dynamic library C++ (`.dll`) đóng vai trò là custom mutator của WinAFL. Khi WinAFL cần tạo một input mới, nó sẽ gọi hàm `dll_mutate_testcase` trong `.dll`, hàm này sẽ gọi vào một Python Script, script này sẽ gọi tiếp Nautilus để sinh ra đoạn script JavaScript mới, rồi sau đó nhúng script này vào file pdf mẫu bằng pikepdf, trả về cho `.dll` rồi `.dll` trả về WinAFL?
   -> Dùng C++ vì WinAFL bắt phải thế và dùng Python Script vì có sẵn pikepdf viết bằng pdf rồi?
2. Nautilus phát triển để hoạt động trên Linux nên gặp khó khăn trong việc gọi Nautilus trên WinAFL.
   -> Do Nautilus sử dụng `fork` system call, các thư viện chuẩn của POSIX như `wait`, `unistd`
