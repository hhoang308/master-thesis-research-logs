#!/bin/bash
# Start the 24h libFuzzer run -- xpdf 4.06 -- Run 2
# Schema: Stage 1+2 + 4 gap fixes (length_delta, skip_compression, no clamp, repeated streams)
# Corpus seeded from run1 corpus (274 files).
# Run from anywhere -- all paths are absolute.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BINARY=/home/hoangnh8/master-thesis-research-logs/research/thesis/pdf-proto-schema/build/pdf_fuzzer
...
echo "Starting pdf_fuzzer (xpdf run 2) at $(date)"
echo "Binary:    $BINARY"
