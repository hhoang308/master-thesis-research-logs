#!/bin/bash
# Start the 24h libFuzzer run -- PoDoFo 0.9.7 -- Run 2
# Schema: Stage 1+2 + 4 gap fixes (length_delta, skip_compression, no clamp, repeated streams)
# Corpus seeded from run1 corpus (52 files).
# Run from anywhere -- all paths are absolute.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BINARY=/home/hoangnh8/master-thesis-research-logs/research/pdf-proto-schema/build/pdf_fuzzer_podofo
...
echo "Starting pdf_fuzzer_podofo (PoDoFo run 2) at $(date)"
echo "Binary:    $BINARY"
...
# Coverage monitor: every 60s parse latest stat line
(
    while kill -0 "$FUZZER_PID" 2>/dev/null; do
        ...
        # Alert if any crash artifact was saved
...
echo "  ls $ARTIFACTS          # check for crash- or timeout- files"
echo ""
